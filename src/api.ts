...
function urlToPath(folder: string, url: string, suffix: string = ''): string {
    ...
    const newFileName = path.join(path.dirname(relativePath), `${urlToName(relativePath)}${suffix}.ts`)
...
export async function createApi({ projectId, folder, requestFactory, urlMapper = s => s, additionalProperties = false }: CreateApiParams) {
...
export async function createModel({
  ...
  `)
        return Promise.all([writeFile(modelPath, modelItf), writeFile(requesterPath, fetcher)])
    } else {
